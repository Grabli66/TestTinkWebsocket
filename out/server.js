...
Main.main = function() {
	...
	handler = new tink_http_middleware_WebSocket(function(req1) {
		req1.stream.forEach(tink_streams__$Stream_Handler_$Impl_$.ofSafe(function(x) {
			console.log("src/Main.hx:27:","GOOD2");
			tink_core__$Callback_CallbackList_$Impl_$.invoke(signal.handlers,tink_streams_Yield.Data(tink_websocket_RawMessage.Text("Hello")));
			return new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(tink_streams_Handled.Resume));
		}));
		return tink_websocket__$MessageStream_MessageStream_$Impl_$.toRawStream(tink_websocket__$MessageStream_MessageStream_$Impl_$.fromRawStream(new tink_streams_SignalStream(signal)));
	}).apply(handler);
	container.run(handler).handle(function(o1) {
		switch(o1._hx_index) {
		case 0:
			var r = o1.running;
			console.log("src/Main.hx:39:","running");
			break;
		case 1:
			var e1 = o1.e;
			console.log("src/Main.hx:45:","failed");
			break;
		case 2:
			console.log("src/Main.hx:42:","shutdown");
			break;
		}
		return;
	});
...
tink_http_middleware_WebSocket.__interfaces__ = [tink_http_MiddlewareObject];
tink_http_middleware_WebSocket.prototype = {
	...
	,apply: function(handler) {
		...
		return new tink_http_SimpleHandler(function(req) {
			...
			var _g = req.body;
			if(tink_websocket__$IncomingHandshakeRequestHeader_IncomingHandshakeRequestHeader_$Impl_$.validate(header)._hx_index == 0 && _g._hx_index == 0) {
				var src = _g.source;
				var ret = _gthis.authenticate(req.header).flatMap(function(o) {
					var v;
					switch(o._hx_index) {
					case 0:
						var this1 = "sec-websocket-key".toLowerCase();
						var header1 = new tink_websocket_OutgoingHandshakeResponseHeader(tink_core_OutcomeTools.sure(header.byName(this1)));
						var _gthis1 = _gthis.ws;
						var req1 = req.clientIp;
						var this2 = tink_io_RealSourceTools.parseStream(src,new tink_websocket_Parser()).map(tink_streams__$Stream_Mapping_$Impl_$.ofPlain(tink_websocket__$Frame_Frame_$Impl_$.fromChunk)).regroup(tink_websocket_MessageRegrouper.get());
						var this3 = _gthis1({ clientIp : req1, header : header, stream : this2});
						console.log("tink/websocket/RawMessageStream.hx:36:","GOOD");
						var this4 = new tink_http__$Response_OutgoingResponseData(header1,tink_websocket__$RawMessageStream_RawMessageStream_$Impl_$.toMaskedChunkStream(this3,function() {
							return null;
						}));
						v = this4;
						break;
					case 1:
						var e = o.failure;
						v = tink_http__$Response_OutgoingResponse_$Impl_$.reportError(e);
						break;
					}
					return new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(v));
				});
	...
	,__class__: tink_http_middleware_WebSocket
};
		...
		if(ctx.request.header.method == "GET") {
			...
			case true:
				if(_g == false) {
					...
					return new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(tink_core_Outcome.Failure(new tink_core_TypedError(404,"Not Found: [" + ctx.request.header.method + "] " + (this1.query == null ? this1.path : this1.path + "?" + this1.query),{ fileName : "src/Main.hx", lineNumber : 18, className : "tink.web.routing.Router0", methodName : "route"}))));
				}
				break;
			}
		} else {
			var this2 = ctx.request.header.url;
			return new tink_core__$Future_SyncFuture(new tink_core__$Lazy_LazyConst(tink_core_Outcome.Failure(new tink_core_TypedError(404,"Not Found: [" + ctx.request.header.method + "] " + (this2.query == null ? this2.path : this2.path + "?" + this2.query),{ fileName : "src/Main.hx", lineNumber : 18, className : "tink.web.routing.Router0", methodName : "route"}))));
		}
	}
...
var tink_websocket__$Frame_Frame_$Impl_$ = {};
tink_websocket__$Frame_Frame_$Impl_$.__name__ = true;
tink_websocket__$Frame_Frame_$Impl_$.toSource = function(this1) {
	return new tink_streams_Single(new tink_core__$Lazy_LazyConst(this1.toChunk()));
};
tink_websocket__$Frame_Frame_$Impl_$.toChunk = function(this1) {
	return this1.toChunk();
};
tink_websocket__$Frame_Frame_$Impl_$.fromChunk = function(v) {
	return tink_websocket_FrameBase.fromChunk(v);
};
tink_websocket__$Frame_Frame_$Impl_$.ofMessage = function(message,maskingKey) {
	var opcode = 0;
	var payload = null;
	...
	case 0:
		var v = message.v;
		opcode = 1;
		payload = haxe_io_Bytes.ofString(v);
		break;
	case 1:
		var b = message.b;
		opcode = 2;
...
tink_websocket__$Frame_Frame_$Impl_$.unmask = function(this1) {
	var _g = this1.payload;
	switch(_g._hx_index) {
	case 0:
		var k = _g.key;
		var p = _g.masked;
		return new tink_websocket_FrameBase(this1.fin,this1.rsv1,this1.rsv2,this1.rsv3,this1.opcode,tink_websocket_Payload.Unmasked(tink_websocket_Masker.mask(p,k)));
	case 1:
		return this1;
	}
};
tink_websocket__$Frame_Frame_$Impl_$.mask = function(this1,key) {
	var _g = this1.payload;
	switch(_g._hx_index) {
	case 0:
		return this1;
	case 1:
		var p = _g.unmasked;
		return new tink_websocket_FrameBase(this1.fin,this1.rsv1,this1.rsv2,this1.rsv3,this1.opcode,tink_websocket_Payload.Masked(tink_websocket_Masker.mask(p,key),key));
	}
};
...
tink_websocket_FrameBase.fromChunk = function(chunk) {
	...
	if(mask) {
		...
		pos += 4;
		maskingKey = tink_websocket__$MaskingKey_MaskingKey_$Impl_$.ofChunk(tink_chunk_ByteChunk.of(key));
	} else {
		maskingKey = null;
	}
...
tink_websocket_FrameBase.prototype = {
	toChunk: function() {
		...
		if(tmp3 >= 126) {
			...
			out.addByte(tmp4 >> 8 & 255);
			var _g4 = this.payload;
			var tmp5;
			switch(_g4._hx_index) {
			case 0:
				var p8 = _g4.masked;
			...
			out.addByte(tmp5 & 255);
		}
		if(this.payload._hx_index == 0) {
			var this1;
			var _g5 = this.payload;
	...
	,get_unmaskedPayload: function() {
		...
		case 0:
			var k = _g.key;
			var p = _g.masked;
			return tink_websocket_Masker.mask(p,k);
		case 1:
			var p1 = _g.unmasked;
			return p1;
		}
	}
	,__class__: tink_websocket_FrameBase
};
var tink_websocket_Payload = $hxEnums["tink.websocket.Payload"] = { __ename__ : true, __constructs__ : ["Masked","Unmasked"]
	,Masked: ($_=function(masked,key) { return {_hx_index:0,masked:masked,key:key,__enum__:"tink.websocket.Payload",toString:$estr}; },$_.__params__ = ["masked","key"],$_)
	,Unmasked: ($_=function(unmasked) { return {_hx_index:1,unmasked:unmasked,__enum__:"tink.websocket.Payload",toString:$estr}; },$_.__params__ = ["unmasked"],$_)
};
...
tink_websocket_Masker.unmask = function(masked,key) {
	return tink_websocket_Masker.mask(masked,key);
};
var tink_websocket__$IncomingHandshakeRequestHeader_IncomingHandshakeRequestHeader_$Impl_$ = {};
tink_websocket__$IncomingHandshakeRequestHeader_IncomingHandshakeRequestHeader_$Impl_$.__name__ = true;
tink_websocket__$IncomingHandshakeRequestHeader_IncomingHandshakeRequestHeader_$Impl_$.validate = function(this1) {
	var errors = [];
	var ensureHeader = function(name,value) {
		var this2 = name.toLowerCase();
		var _g = this1.byName(this2);
		switch(_g._hx_index) {
		case 0:
			var v = _g.data;
			if(value == null) {
			...
			var items = v.split(",").map(function(x) {
				return StringTools.trim(x).toLowerCase();
			});
			if(items.indexOf(value) > -1) {
				return;
			}
			errors.push("Header value for " + name + " is expected to be " + value + ", but got " + v);
			break;
		case 1:
			var f = _g.failure;
	...
	if(errors.length > 0) {
		return tink_core_Outcome.Failure(tink_core_TypedError.withData(null,"Invalid request header",errors,{ fileName : "tink/websocket/IncomingHandshakeRequestHeader.hx", lineNumber : 37, className : "tink.websocket._IncomingHandshakeRequestHeader.IncomingHandshakeRequestHeader_Impl_", methodName : "validate"}));
	} else {
		return tink_core_Outcome.Success(tink_core_Noise.Noise);
	}
};
tink_websocket__$IncomingHandshakeRequestHeader_IncomingHandshakeRequestHeader_$Impl_$.get_key = function(this1) {
	var this2 = "sec-websocket-key".toLowerCase();
	return tink_core_OutcomeTools.sure(this1.byName(this2));
};
tink_websocket__$IncomingHandshakeRequestHeader_IncomingHandshakeRequestHeader_$Impl_$.parser = function() {
	return tink_http_IncomingRequestHeader.parser();
};
var tink_websocket__$MaskingKey_MaskingKey_$Impl_$ = {};
tink_websocket__$MaskingKey_MaskingKey_$Impl_$.__name__ = true;
tink_websocket__$MaskingKey_MaskingKey_$Impl_$._new = function(a,b,c,d) {
	var bytes = new haxe_io_Bytes(new ArrayBuffer(4));
	bytes.b[0] = a;
	bytes.b[1] = b;
	bytes.b[2] = c;
	bytes.b[3] = d;
	var this1 = tink_chunk_ByteChunk.of(bytes);
	return this1;
};
tink_websocket__$MaskingKey_MaskingKey_$Impl_$.ofChunk = function(c) {
	if(c.getLength() != 4) {
		throw new js__$Boot_HaxeError("Invalid key length, should be 4");
	}
	return c;
};
tink_websocket__$MaskingKey_MaskingKey_$Impl_$.random = function() {
	return tink_websocket__$MaskingKey_MaskingKey_$Impl_$._new(Std.random(256),Std.random(256),Std.random(256),Std.random(256));
};
var tink_websocket_Message = $hxEnums["tink.websocket.Message"] = { __ename__ : true, __constructs__ : ["Text","Binary"]
	,Text: ($_=function(v) { return {_hx_index:0,v:v,__enum__:"tink.websocket.Message",toString:$estr}; },$_.__params__ = ["v"],$_)
	,Binary: ($_=function(b) { return {_hx_index:1,b:b,__enum__:"tink.websocket.Message",toString:$estr}; },$_.__params__ = ["b"],$_)
};
var tink_websocket__$MessageStream_MessageStream_$Impl_$ = {};
tink_websocket__$MessageStream_MessageStream_$Impl_$.__name__ = true;
tink_websocket__$MessageStream_MessageStream_$Impl_$.fromRawStream = function(raw) {
	return raw.regroup(tink_streams__$Stream_Regrouper_$Impl_$.ofIgnoranceSync(function(m) {
		var _g = m[0];
		var tmp;
		switch(_g._hx_index) {
		case 0:
			var v = _g.v;
			tmp = tink_streams__$Stream_Stream_$Impl_$.single(tink_websocket_Message.Text(v));
			break;
		case 1:
			var v1 = _g.b;
			tmp = tink_streams__$Stream_Stream_$Impl_$.single(tink_websocket_Message.Binary(v1));
			break;
		default:
			tmp = tink_streams_Empty.inst;
		}
		return tink_streams_RegroupResult.Converted(tmp);
	}));
};
tink_websocket__$MessageStream_MessageStream_$Impl_$.toRawStream = function(this1) {
	return this1.regroup(tink_streams__$Stream_Regrouper_$Impl_$.ofIgnoranceSync(function(m) {
		var _g = m[0];
		var s;
		switch(_g._hx_index) {
		case 0:
			var v = _g.v;
			s = tink_streams__$Stream_Stream_$Impl_$.single(tink_websocket_RawMessage.Text(v));
			break;
		case 1:
			var v1 = _g.b;
			s = tink_streams__$Stream_Stream_$Impl_$.single(tink_websocket_RawMessage.Binary(v1));
			break;
		}
		return tink_streams_RegroupResult.Converted(s);
	}));
};
var tink_websocket_OutgoingHandshakeResponseHeader = function(key,fields) {
	...
	var fillHeader = function(name,value) {
		...
		if(_gthis.byName(this1)._hx_index == 1) {
			var this2 = name.toLowerCase();
			_gthis.fields.push(new tink_http_HeaderField(this2,value));
		}
	};
...
tink_websocket_OutgoingHandshakeResponseHeader.prototype = $extend(tink_http_ResponseHeaderBase.prototype,{
	__class__: tink_websocket_OutgoingHandshakeResponseHeader
});
...
tink_websocket_Parser.__interfaces__ = [tink_io_StreamParserObject];
tink_websocket_Parser.prototype = {
	...
	,progress: function(cursor) {
		...
		case -2:case -1:
			...
			this.out = tink__$Chunk_Chunk_$Impl_$.catChunk(this.out,cursor.left());
			return tink_io_ParseStep.Progressed;
		case 0:
			cursor.next();
			var secondByte = cursor.currentByte;
			this.mask = secondByte >> 7 == 1;
			var _g2 = secondByte & 127;
			var tmp;
			switch(_g2) {
			case 126:
				this.length = -1;
			...
			default:
				var len = _g2;
				this.length = len + 2 + (this.mask ? 4 : 0);
				tmp = this.length - 2;
			}
			this.required = tmp;
			cursor.next();
			...
			return tink_io_ParseStep.Progressed;
		default:
			var ret = tink_io_ParseStep.Done(tink__$Chunk_Chunk_$Impl_$.catChunk(this.out,cursor.right().slice(0,this.required)));
	...
	,reset: function() {
		this.out = tink__$Chunk_Chunk_$Impl_$.EMPTY;
		this.length = 0;
		this.required = 2;
	}
	,__class__: tink_websocket_Parser
};
var tink_websocket_RawMessage = $hxEnums["tink.websocket.RawMessage"] = { __ename__ : true, __constructs__ : ["Text","Binary","ConnectionClose","Ping","Pong"]
	,Text: ($_=function(v) { return {_hx_index:0,v:v,__enum__:"tink.websocket.RawMessage",toString:$estr}; },$_.__params__ = ["v"],$_)
	,Binary: ($_=function(b) { return {_hx_index:1,b:b,__enum__:"tink.websocket.RawMessage",toString:$estr}; },$_.__params__ = ["b"],$_)
	,ConnectionClose: {_hx_index:2,__enum__:"tink.websocket.RawMessage",toString:$estr}
	,Ping: ($_=function(b) { return {_hx_index:3,b:b,__enum__:"tink.websocket.RawMessage",toString:$estr}; },$_.__params__ = ["b"],$_)
	,Pong: ($_=function(b) { return {_hx_index:4,b:b,__enum__:"tink.websocket.RawMessage",toString:$estr}; },$_.__params__ = ["b"],$_)
};
var tink_websocket__$RawMessageStream_RawMessageStream_$Impl_$ = {};
tink_websocket__$RawMessageStream_RawMessageStream_$Impl_$.__name__ = true;
tink_websocket__$RawMessageStream_RawMessageStream_$Impl_$.append = function(this1,other) {
	return this1.append(other);
};
tink_websocket__$RawMessageStream_RawMessageStream_$Impl_$.prepend = function(this1,other) {
	return this1.prepend(other);
};
tink_websocket__$RawMessageStream_RawMessageStream_$Impl_$.filter = function(this1,f) {
	return this1.filter(f);
};
tink_websocket__$RawMessageStream_RawMessageStream_$Impl_$.blend = function(this1,other) {
	return this1.blend(other);
};
tink_websocket__$RawMessageStream_RawMessageStream_$Impl_$.lift = function(s) {
	return s;
};
tink_websocket__$RawMessageStream_RawMessageStream_$Impl_$.ofChunkStream = function(s) {
	return s.map(tink_streams__$Stream_Mapping_$Impl_$.ofPlain(tink_websocket__$Frame_Frame_$Impl_$.fromChunk)).regroup(tink_websocket_MessageRegrouper.get());
};
tink_websocket__$RawMessageStream_RawMessageStream_$Impl_$.ofFrameStream = function(s) {
	return s.regroup(tink_websocket_MessageRegrouper.get());
};
tink_websocket__$RawMessageStream_RawMessageStream_$Impl_$.toUnmaskedChunkStream = function(this1) {
	console.log("tink/websocket/RawMessageStream.hx:36:","GOOD");
	return tink_websocket__$RawMessageStream_RawMessageStream_$Impl_$.toMaskedChunkStream(this1,function() {
		return null;
	});
};
tink_websocket__$RawMessageStream_RawMessageStream_$Impl_$.toUnmaskedFrameStream = function(this1) {
	return tink_websocket__$RawMessageStream_RawMessageStream_$Impl_$.toMaskedFrameStream(this1,function() {
		return null;
	});
};
tink_websocket__$RawMessageStream_RawMessageStream_$Impl_$.toMaskedChunkStream = function(this1,key) {
	console.log("tink/websocket/RawMessageStream.hx:44:","SHIT1");
	return tink_websocket__$RawMessageStream_RawMessageStream_$Impl_$.toMaskedFrameStream(this1,key).map(tink_streams__$Stream_Mapping_$Impl_$.ofPlain(function(f) {
		console.log("tink/websocket/RawMessageStream.hx:46:","SHIT2");
		return f.toChunk();
	}));
};
tink_websocket__$RawMessageStream_RawMessageStream_$Impl_$.toMaskedFrameStream = function(this1,key) {
	console.log("tink/websocket/RawMessageStream.hx:52:","BAD1");
	return this1.map(tink_streams__$Stream_Mapping_$Impl_$.ofPlain(function(message) {
		console.log("tink/websocket/RawMessageStream.hx:54:","BAD2");
		return tink_websocket__$Frame_Frame_$Impl_$.ofMessage(message,key());
	}));
};
var tink_websocket_MessageRegrouper = function() { };
tink_websocket_MessageRegrouper.__name__ = true;
tink_websocket_MessageRegrouper.transform = function(s) {
	return s.map(tink_streams__$Stream_Mapping_$Impl_$.ofPlain(tink_websocket__$Frame_Frame_$Impl_$.fromChunk)).regroup(tink_websocket_MessageRegrouper.get());
};
...
tink_websocket_MessageRegrouper.inst = tink_streams__$Stream_Regrouper_$Impl_$.ofFuncSync(function(frames,s) {
	var last = frames[frames.length - 1];
	if(!last.fin) {
		return tink_streams_RegroupResult.Untouched;
	}
	var mergeBytes = function() {
		var out = tink__$Chunk_Chunk_$Impl_$.EMPTY;
		var _g = 0;
		while(_g < frames.length) {
			var frame = frames[_g];
			++_g;
			out = tink__$Chunk_Chunk_$Impl_$.catChunk(out,frame.get_unmaskedPayload());
		}
		return out;
	};
	var tmp;
	...
	return tink_streams_RegroupResult.Converted(tink_streams__$Stream_Stream_$Impl_$.single(tmp));
});
Main.main();
